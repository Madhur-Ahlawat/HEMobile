...
import com.conduent.nationalhighways.databinding.AdapterPaymentCardBinding

class PaymentCardAdapter(
    private val context: Context?,
    var list: MutableList<CardListResponseModel?>?,
    var listner: (Boolean?, Int?, CardListResponseModel?) -> Unit
    ...
    inner class MyViewHolder(var binding: AdapterPaymentCardBinding) :
            ...
            data?.run {
                binding.apply {
                    ...
                    spannableString.setSpan(
                        ...
                        spannableString.length - (cardNumber?.length ?: 0),
        ...
        override fun onClick(v: View?) {
            for (i in list?.indices!!) {
                if (absoluteAdapterPosition != i) list?.get(i)?.check = false
            }
            list?.get(absoluteAdapterPosition)?.check =
                !(list?.get(absoluteAdapterPosition)?.check ?: false)
            listner.invoke(
                list?.get(absoluteAdapterPosition)?.check,
                absoluteAdapterPosition,
                list?.get(absoluteAdapterPosition)
            )
