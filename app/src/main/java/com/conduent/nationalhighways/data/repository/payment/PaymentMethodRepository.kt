...
class PaymentMethodRepository @Inject constructor(private val apiService: ApiService) {
    ...
    suspend fun editDefaultCard(model: PaymentMethodEditModel?) =
        apiService.editDefaultCard(model = model)

    suspend fun saveNewCard(model: AddCardModel?) = apiService.savedNewCard(model = model)

    suspend fun saveDirectDebitNewCard(model: SaveNewCardRequest?) =
        apiService.saveDirectDebitNewCard(model = model)
