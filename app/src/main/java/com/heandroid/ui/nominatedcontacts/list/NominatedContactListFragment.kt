...
class NominatedContactListFragment : BaseFragment<FragmentNominatedContactListBinding>(), View.OnClickListener, NominatedContactListener {
    ...
    override fun getFragmentBinding(inflater: LayoutInflater, container: ViewGroup?): FragmentNominatedContactListBinding = FragmentNominatedContactListBinding.inflate(inflater,container,false)
    ...
    override fun onClick(v: View?) {
        when(v?.id){
            R.id.btnActive ->{
                ...
                binding.btnActive.backgroundTintList=requireActivity().getColorStateList(R.color.green)

                binding.btnInvited.changeTextColor(R.color.green)
                binding.btnInvited.changeBackgroundColor(R.color.white)
                binding.btnInvited.backgroundTintList=requireActivity().getColorStateList(R.color.white)
            }
            R.id.btnInvited ->{
                ...
                binding.btnActive.backgroundTintList=requireActivity().getColorStateList(R.color.white)

                binding.btnInvited.changeTextColor(R.color.white)
                binding.btnInvited.changeBackgroundColor(R.color.green)
                binding.btnInvited.backgroundTintList=requireActivity().getColorStateList(R.color.green)

            }

            R.id.btnNominatedContact ->{
                if((list?.size?:0)>=5) showError(binding.root,getString(R.string.str_nominated_contacts_limit_reached))
    ...
    private fun setDataToUI(list: List<SecondaryAccountData?>?){
        ...
        val nominatedAdapter =NominatedContactsAdapter(requireContext(), this)
        nominatedAdapter.setList(list)
            ...
            when(status){
                is Resource.Success -> {
                        if (status.data?.accessRights?.accessVo?.isNotEmpty()==true){
                            if(status.data.accessRights.accessVo[0].value.equals("READ", true)) filterList?.get(selectedPosition)?.mPermissionLevel = "Read Only"
                            else filterList?.get(selectedPosition)?.mPermissionLevel = "Amend Account, Vehicle data"
                        }else filterList?.get(selectedPosition)?.mPermissionLevel = "Amend Account, Vehicle data"


                        binding.rvList.adapter?.notifyItemChanged(selectedPosition,list.get(selectedPosition))
                }
                is Resource.DataError -> { showError(binding.root,status.errorMsg) }
            }
        }catch (e: Exception){}
    ...
    override fun onItemClick(type: String, data: SecondaryAccountData, pos: Int, isExpanded: Boolean) {
        when (type) {
            ...
            "Edit" -> {
                val model=CreateAccountRequestModel(firstName = data.firstName, lastName = data.lastName, emailId = data.emailAddress,
                                                    phoneNumber = data.phoneNumber, accountId =data.secAccountRowId, status = data.accountStatus)
                val bundle= Bundle()
                bundle.putBoolean("edit",true)
                bundle.putParcelable("data",model)
                findNavController().navigate(R.id.action_ncListFragment_to_ncFullNameFragment,bundle)
            }
        }
    }
}