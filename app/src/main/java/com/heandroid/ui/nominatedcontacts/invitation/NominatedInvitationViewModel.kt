...
@HiltViewModel
class NominatedInvitationViewModel @Inject constructor(private val repository: NominatedContactsRepo) : BaseViewModel() {
    ...
    fun createAccount(model: CreateAccountRequestModel?) {
        viewModelScope.launch {
            try {
                _createAccount.postValue(success(repository.createSecondaryAccount(model), errorManager))
            } catch (e: Exception) {
    ...
    fun updateAccessRight(model: UpdateAccessRightModel?) {
        viewModelScope.launch {
            try {
                _updateAccessRight.postValue(success(repository.updateAccessRight(model), errorManager))
            } catch (e: Exception) {
    ...
    fun updateSecondaryAccountData(body: CreateAccountRequestModel?) {
        viewModelScope.launch {
            try {
                _updateAccount.postValue(success(repository.updateSecondaryAccount(body), errorManager))
            } catch (e: Exception) {
    ...
    fun validationEmail(model: CreateAccountRequestModel?): Pair<Boolean,String>{
        var ret= Pair(true,"")
        if(model?.emailId?.isEmpty()==true) ret=Pair(false, BaseApplication.INSTANCE.getString(R.string.error_email))
        else if(!Patterns.EMAIL_ADDRESS.matcher(model?.emailId).matches()) ret=Pair(false, BaseApplication.INSTANCE.getString(R.string.error_valid_email))
        return ret
    }
