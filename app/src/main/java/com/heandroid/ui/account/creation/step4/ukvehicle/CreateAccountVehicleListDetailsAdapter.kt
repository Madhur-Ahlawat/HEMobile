...
import com.heandroid.ui.vehicle.vehiclelist.VehicleDetailsAdapter
import com.heandroid.utils.DateUtils

class CreateAccountVehicleListDetailsAdapter(
    private val mContext: Context,
    private val onItemClick: ItemClickListener? = null
) : RecyclerView.Adapter<CreateAccountVehicleListDetailsAdapter.VrmHeaderViewHolder>() {
...
//            arrowImg.animate().rotation(180f).start()
            ...
            for (i in 0..5) {
                when (i) {
                    ...
                    4 -> {
                        val mem2 = VehicleTitleAndSub("Country of registration", vehicleItem?.newPlateInfo?.country)
                        mList.add(mem2)
                    }
                }
            }
