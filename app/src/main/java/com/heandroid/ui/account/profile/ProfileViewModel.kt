...
class ProfileViewModel @Inject constructor(private val repository: ProfileRepository): BaseViewModel()  {
    ...
    fun emailValidationForUpdatation(model : ProfileUpdateEmailModel?){
        viewModelScope.launch {
            try {
                _emailValidation.postValue(success(repository.emailValidationForUpdation(model), errorManager))
            } catch (e: Exception) {
    ...
    fun checkPassword(model: UpdateAccountPassword?): Pair<Boolean,String>{
        var ret= Pair(true,"")
        if(!Utils.isValidPassword(model?.newPassword)) ret=Pair(false,BaseApplication.INSTANCE.getString(R.string.confirm_password_validation))
