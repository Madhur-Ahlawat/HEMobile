package com.heandroid.ui.customviews
...
class BottomNavigationView @JvmOverloads constructor(
     ...
     */
    private val params = BottomNavParams()
    ...
    init {
        ...
        if (a.hasValue(R.styleable.BottomNavigationView_active_color)) {
            val activeColor = a.getColor(R.styleable.BottomNavigationView_active_color, 0)
            params.activeColor = activeColor
        }

        if (a.hasValue(R.styleable.BottomNavigationView_passive_color)) {
            val passiveColor = a.getColor(R.styleable.BottomNavigationView_passive_color, 0)
            params.passiveColor = passiveColor
        }

        if (a.hasValue(R.styleable.BottomNavigationView_pressed_color)) {
            val pressedColor = a.getColor(R.styleable.BottomNavigationView_pressed_color, 0)
            params.pressedColor = pressedColor
        }

        if (a.hasValue(R.styleable.BottomNavigationView_item_padding)) {
            val px = a.getDimension(R.styleable.BottomNavigationView_item_padding, 0f)
            params.itemPadding = px
        }

        if (a.hasValue(R.styleable.BottomNavigationView_item_text_size)) {
            val px = a.getDimensionPixelSize(R.styleable.BottomNavigationView_item_text_size, 0)
            params.itemTextSize = px.toFloat()
        }

        if (a.hasValue(R.styleable.BottomNavigationView_animation_duration)) {
            val animDuration = a.getInteger(R.styleable.BottomNavigationView_animation_duration, 0)
            params.animationDuration = animDuration
        }

        if (a.hasValue(R.styleable.BottomNavigationView_scale_percent)) {
            val scalePercent = a.getInteger(R.styleable.BottomNavigationView_scale_percent, 0)
